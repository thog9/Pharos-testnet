...
LANG = {
    'vi': {
        ...
        'input_mint_count': 'Nhập số lượng NFT muốn mint: ',
        ...
        'success': '✅ Mint NFT thành công Pharos Gotchipus!',
        'failure': '❌ Mint NFT thất bại Pharos Gotchipus',
    ...
    'en': {
        ...
        'input_mint_count': 'Enter the number of NFTs to mint: ',
        ...
        'success': '✅ Successfully minted NFT Pharos Gotchipus!',
        'failure': '❌ Failed to mint NFT Pharos Gotchipus',
...
# Hàm mint NFT
async def mint_nft(w3: Web3, private_key: str, wallet_index: int, mint_count: int, proxy: str = None, language: str = 'en'):
...
# Hàm xử lý từng ví
async def process_wallet(index: int, profile_num: int, private_key: str, mint_count: int, proxy: str, w3: Web3, language: str):
    results = []
    for i in range(mint_count):
        print_border(f"Mint NFT {i+1}/{mint_count}", Fore.YELLOW)
        result = await mint_nft(w3, private_key, profile_num, mint_count, proxy, language)
...
# Hàm chính
async def run_mintgotchipus(language: str = 'vi'):
    print()
    ...
    # Nhập số lượng NFT muốn mint
    while True:
        try:
            mint_count = int(input(f"{Fore.CYAN}{LANG[language]['input_mint_count']}{Style.RESET_ALL}"))
            if mint_count > 0:
                break
            print(f"{Fore.YELLOW}  {LANG[language]['invalid_input']}{Style.RESET_ALL}")
        except ValueError:
            print(f"{Fore.YELLOW}  {LANG[language]['invalid_input']}{Style.RESET_ALL}")

    successful_mints = 0
    total_mints = len(private_keys) * mint_count
    ...
    async def process_wallet_task(index, profile_num, private_key, proxy):
        ...
        async with semaphore:
            result = await process_wallet(index, profile_num, private_key, mint_count, proxy, w3, language)
            if result:
                successful_mints += mint_count
...
if __name__ == "__main__":
    asyncio.run(run_mintgotchipus('vi'))
